    ...
    enableDisplayToggle(){
        ...
        displayToggle.addEventListener('click', () => {
            if(this.focus){
                ...
                global.geotab.addin.addinMediaFiles.blur();
            } else {
                displayToggle.innerHTML = 'Blur add-in';
                global.geotab.addin.addinMediaFiles.focus(global.api, global.state);
            }
    ...
    listenToHash(){
        ...
        setInterval(()=>{
            if(window.location.hash !== this.storedHash){
                this.storedHash = window.location.hash;
                if(this.storedHash !== '#' + 'addinMediaFiles'){
                    geotab.addin.addinMediaFiles.blur();
                } else {
                    geotab.addin.addinMediaFiles.focus(global.api, global.state);
                }
            }
        }, 100);
    }
